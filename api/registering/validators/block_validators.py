...
class ColoredSolidBlockValidator(Validator):
    def validate(self, arg_name: str, value) -> ValidationResult:
        ...
        if not value.startswith("minecraft:"):
            return ValidationResult(False, f"{arg_name} is not a valid block")
        ...
        if not has_valid_color:
            return ValidationResult(False, f"{arg_name} is not a valid color")
        if value != f"minecraft:{color_name}_{block_name}":
            return ValidationResult(False, f"{arg_name} is not a valid colored solid block")
